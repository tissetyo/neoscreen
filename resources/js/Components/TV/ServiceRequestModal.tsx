...
export default function ServiceRequestModal({ isOpen, onClose, onOrderComplete }: ServiceRequestModalProps) {
  ...
  // Enable native D-pad navigation for this modal
  useDpadNavigation({ enabled: isOpen && !showQr, onEscape: onClose, selector: '.tv-focusable' });
  ...
  // Reset current index when step changes
  useEffect(() => {
    if (step === 'items') setCurrentIndex(0); // Optional: keep state clean
  }, [step]);
  ...
  const handleConfirmOrder = async () => {
    ...
    await fetch(`/api/room/${store.roomId}/service-request`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...authHeaders },
      body: JSON.stringify({
        ...
        >
          ...
          <AnimatePresence>
            {step === 'done' && (
              ...
              >
                <CheckCircle2 className="w-20 h-20 text-[#d4af37] mb-4 animate-bounce" />
                <h2 className="text-3xl font-bold text-white mb-2">Order Sent!</h2>
                <p className="text-xl text-[#f3e5ab]/70 text-center max-w-md">Your order has been sent to the Front Desk. Opening chat...</p>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <AnimatePresence>
            {showQr && (
              ...
              >
                ...
                >
                  <ConciergeBell className="w-10 h-10 text-[#d4af37] mx-auto mb-3" />
              ...
              <div className="flex items-center gap-[0.6vw]">
                ...
                {totalItems > 0 && step !== 'confirm' && (
                  <button
                    ...
                    className="flex items-center gap-[0.3vw] px-[0.8vw] py-[0.4vh] rounded-full bg-[#d4af37] hover:bg-[#d4af37] transition-colors tv-focusable"
                    tabIndex={0}
                  >
                ...
                {step === 'carousel' ? (
                        ...
                        <div className="relative" style={{ width: '22vw', height: '55vh', perspective: '1200px' }}>
                          {categories.map((cat, idx) => {
                            ...
                            return (
                              <motion.div
                                key={cat.id}
                                className="absolute inset-0 rounded-3xl overflow-hidden cursor-pointer tv-focusable group"
                                tabIndex={0}
                                style={{ zIndex }}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-[0.8vw] mb-[2vh]">
                      <div className="w-[3vw] h-[3vw] rounded-xl bg-[#d4af37]/20 flex items-center justify-center text-[#f3e5ab]">
                    ...
                    <div className="flex-1 overflow-y-auto hide-scrollbar grid grid-cols-2 gap-[0.8vw]">
                      {options.map((opt) => {
                        ...
                        return (
                          <div
                            key={opt.id}
                            className={`p-[1vw] rounded-2xl flex items-center justify-between gap-[0.6vw] tv-focusable transition-all cursor-pointer focus:ring-2 focus:ring-[#d4af37] focus:ring-offset-2 focus:ring-offset-slate-900 group`}
                            style={{
                          ...
                          >
                            <div className="min-w-0 flex-1">
                              <h4 className="text-white text-[0.85vw] font-bold truncate group-focus:text-[#f3e5ab] transition-colors">{opt.name}</h4>
                              <span className="text-[#d4af37] text-[0.7vw] font-medium">{formatRupiah(opt.price)}</span>
                            </div>
                            {qty === 0 ? (
                              <button
                                onClick={(e) => { e.stopPropagation(); updateCart(opt, 1); }}
                                className="w-[2vw] h-[2vw] rounded-full bg-white/10 hover:bg-[#d4af37] flex items-center justify-center text-white transition-colors tv-focusable shrink-0"
                              ...
                              <div className="flex items-center gap-[0.3vw] bg-white/10 rounded-full px-[0.3vw] py-[0.2vw] shrink-0">
                                <button onClick={(e) => { e.stopPropagation(); updateCart(opt, -1); }} className="w-[1.6vw] h-[1.6vw] rounded-full bg-white/10 hover:bg-rose-500 flex items-center justify-center text-white tv-focusable relative z-10" tabIndex={0}>
                                  <Minus className="w-[0.6vw] h-[0.6vw]" />
                                </button>
                                <span className="text-white font-bold text-[0.7vw] min-w-[1vw] text-center">{qty}</span>
                                <button onClick={(e) => { e.stopPropagation(); updateCart(opt, 1); }} className="w-[1.6vw] h-[1.6vw] rounded-full bg-[#d4af37] hover:bg-[#d4af37] flex items-center justify-center text-white tv-focusable relative z-10" tabIndex={0}>
                    ...
                    {cart.length > 0 && (
                      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="mt-[1.5vh] pt-[1.5vh] border-t border-white/10">
                        <button
                          onClick={() => setStep('confirm')}
                          className="w-full bg-[#d4af37] hover:bg-[#d4af37] text-white font-bold py-[1.2vh] rounded-xl flex items-center justify-center gap-[0.6vw] transition-colors tv-focusable text-[0.9vw]"
                          tabIndex={0}
                        >
                  ...
                  >
                        ...
                        <div key={item.option.id} className="flex items-center justify-between p-[0.8vw] rounded-xl" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.12)' }}>
                          ...
                          <div className="flex items-center gap-[0.8vw] ml-[1vw]">
                            <div className="flex items-center gap-[0.3vw]">
                              ...
                              <button onClick={() => updateCart(item.option, 1)} className="w-[1.6vw] h-[1.6vw] rounded-full bg-[#d4af37]/80 hover:bg-[#d4af37] flex items-center justify-center text-white tv-focusable" tabIndex={0}>
                                <Plus className="w-[0.5vw] h-[0.5vw]" />
                              </button>
                            </div>
                            <span className="text-[#d4af37] font-bold text-[0.7vw] w-[5vw] text-right">{formatRupiah(item.option.price * item.quantity)}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="border-t border-white/10 pt-[1.5vh] mt-[1.5vh]">
                      ...
                      <div className="flex gap-[0.6vw]">
                        ...
                        <button onClick={handleConfirmOrder} disabled={submitting} className="flex-1 py-[1vh] rounded-xl bg-[#d4af37] text-white font-bold hover:bg-[#d4af37] transition-colors tv-focusable disabled:opacity-50 text-[0.8vw]" tabIndex={0}>
