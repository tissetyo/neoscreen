      ...
      >
        ...
        >
          ...
          <div className="flex justify-center gap-4 mb-[3vh]">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className={`w-[1vw] h-[1vw] rounded-full ${i < pin.length ? 'bg-[#d4af37]' : 'bg-white/30'}`} />
            ))}
          </div>
            ...
            {['1','2','3','4','5','6','7','8','9','Cancel','0','⌫'].map((key) => (
              <button key={key} disabled={!key || loading}
                ...
                className={`h-[3.5vw] rounded-xl text-[1.4vw] font-medium transition-colors 
                  ${key ? 'bg-white/10 hover:bg-[#d4af37]/60 active:bg-[#aa8529] text-white logout-focusable focus:ring-4 focus:ring-[#d4af37]/50' : 'invisible'}`}
                tabIndex={key ? 0 : -1}>
                {key}
              </button>
            ))}
